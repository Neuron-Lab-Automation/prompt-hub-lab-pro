...
export function useSupabase() {
  ...
  useEffect(() => {
    if (DISABLE_SUPABASE) {
      // Check if user should be logged in (for development)
      const shouldBeLoggedIn = localStorage.getItem('mockUserLoggedIn') === 'true';
      if (shouldBeLoggedIn) {
        setUser({
          id: 'mock-user-id',
...
export function usePrompts() {
  ...
  const toggleFavorite = async (promptId: string) => {
    console.log('Mock toggle favorite:', { promptId });
    ...
    setPrompts(prevPrompts => {
      ...
      console.log('Updated prompts:', updatedPrompts.find(p => p.id === promptId));
